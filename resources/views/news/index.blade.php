...
<div class="bg-white px-4 py-5 border-b border-gray-200 sm:px-6">
    <div class="-ml-4 -mt-2 flex items-center justify-between flex-wrap sm:flex-no-wrap">
        <div class="ml-4 mt-2">
            <h1 class="text-2xl font-semibold text-gray-900">Новости</h1>
        </div>
        <div class="ml-4 mt-2 flex-shrink-0">
            <a href="{{route('news.create')}}" class="">
                    ...
                    @foreach($news as $item)
                        <tr>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 leading-5 font-medium text-gray-900">
                                <a href="{{route('news.show', $item->id)}}" class="mr-2 inline-flex" target="_blank">
{{--                                    <img src="{{asset('/storage/' . $item->image)}}" alt="" width="160" height="120">--}}
                                    <img src="{{$item->getFirstMediaUrl('news-image', 'thumb')}}" alt=""/>
                                </a>
                            </td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 leading-5 font-medium text-gray-900">
                                <a href="{{route('news.show', $item->id)}}" class="mr-2 inline-flex" target="_blank">
    ...
    {{--                                    <a href="#" class="text-indigo-600 hover:text-indigo-900 focus:outline-none focus:underline">Edit</a>--}}
                                <a href="{{route('news.edit', $item->id)}}" class="mr-2 inline-flex">
                                ...
                                <form action="{{route('news.destroy', $item->id)}}" method="post" onsubmit="return confirm('Вы уверены?');" class=" inline-flex">
                                    @method('delete')
